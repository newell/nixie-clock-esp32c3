...
void sync_sntp(void)
{
        ...
        if (buffer != NULL) {
            ...
            if (time_json != NULL && cJSON_IsObject(time_json)) {
                ...
                if (timezone_json != NULL && cJSON_IsString(timezone_json)) {
                    strncpy(timezone_str, timezone_json->valuestring, sizeof(timezone_str));
                }
            }
            cJSON_Delete(root);
            free(buffer);
    ...
    // Set timezone based on UTC offset
    char tz_str[32];
    snprintf(tz_str, sizeof(tz_str), "%s", timezone_str);
    ESP_LOGI(TAG, "tz_str: %s", tz_str); // Check
    setenv("TZ", tz_str, 1);
    ESP_LOGI(TAG, "TZ environment variable set to: %s", getenv("TZ")); // Debug log
    tzset();
    ESP_LOGI(TAG, "TZ environment variable set to (after tzset): %s", getenv("TZ")); // Debug log
